...
def generate(model,tokenizer,prompt,max_new_tokens=100,temperature=0.7):
    ids=tokenizer.encode(prompt,max_length=model.cfg.max_seq_len)
    ...
    for _ in range(max_new_tokens):
        ...
        if next_id.item()==tokenizer.vocab["<eos>"]: break
    return tokenizer.decode(x[0,len(ids):].tolist())

def main():
    p=argparse.ArgumentParser(); p.add_argument("--model-dir",default="outputs/xen")
    p.add_argument("--prompt",required=True); p.add_argument("--max-new-tokens",type=int,default=100)
    p.add_argument("--temperature",type=float,default=0.7); a=p.parse_args()
